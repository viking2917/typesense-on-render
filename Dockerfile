...
RUN echo '#!/bin/bash\nmkdir -p /tmp/typesense-data\nexec /opt/typesense-server --data-dir /tmp/typesense-data --api-key "$TYPESENSE_API_KEY" --enable-cors --api-address 0.0.0.0 --api-port "$PORT"' > /start.sh && chmod +x /start.sh
